...
// from the dirty map.
var expunged = unsafe.Pointer(new(Unit))
...
func (m *SyncDict[K, V]) Load(key K) (value V, ok bool) {
	read := m.loadReadOnly()
	e, ok := read.m[key]
	if !ok && read.amended {
		m.mu.Lock()
...
func (m *SyncDict[K, V]) LoadOrStore(key K, value V) (actual V, loaded bool) {
...
func (m *SyncDict[K, V]) LoadAndDelete(key K) (value V, loaded bool) {
	read := m.loadReadOnly()
	e, ok := read.m[key]
	if !ok && read.amended {
		m.mu.Lock()
